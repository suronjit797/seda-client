...
const Header = ({ handle, isDark, setIsDark }) => {
  ...
  const getDevices = async (locationId) => {
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/device/site/` + locationId,
      { withCredentials: true }
    );
    if (response) {
  ...
  const handleSiteChange = async (e) => {
    ...
    const response = await axios.get(
      `${process.env.REACT_APP_API_URL}/site-location/` + locationId,
      { withCredentials: true }
    );
    if (response) {
      ...
      <div className="container-fluid py-2 text-white">
        <div className="row d-flex align-items-center">
          ...
          <div className="col-md-2">
            <div>
              {(location.pathname === "/" ||
                location.pathname === "/analysis-reporting") &&
                (() => {
                  switch (userDetails?.role) {
                    case "user":
                      return (
                        <select
                          ...
                          disabled
                        >
                          <option value={userDetails?.site?._id}>
                            {userDetails?.site?.name}
                          </option>
                        </select>
                      );
                    case "public":
